...
def generate_MDP_input(sample_data, feature_name):
#     sample_data = pandas.read_csv(filename)
#     feature_name = list(sample_data)
    ...
    sample_data['state'] = sample_data[features].apply(lambda x: ':'.join(str(v) for v in x), axis=1)
    # sample_data['state'] = sample_data[features].apply(tuple, axis=1)
    students_variables = students_variables + ['state']
    data = sample_data[students_variables]
...
def output_policy(distinct_acts, distinct_states, vi):
    ...
    for s in range(Ns):
        action = distinct_acts[vi.policy[s]]
        print(distinct_states[s]+ " -> " + action + ", "+str(vi.V[s]))
        
        if action in action_cnts:
            action_cnts[action] += 1
        else:
            action_cnts[action] = 1
        
    print("\nTotal number of the distinct states: {}".format(Ns))
    print(" - The number of actions: {}".format(action_cnts))
