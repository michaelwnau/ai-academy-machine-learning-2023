...
class hmm:
    def __init__(self, n_state, obs_symbols, **args):
        ...
        :param args: 'Pi' - matrix of initial state probability distribution
                     'T' - matrix of transition probability
        ...
        if 'T' in args:
            ...
            if numpy.shape(self.T) != (self.N, self.N):
                raise ValueError("The transition probability matrix dimension mismatches the given states number.")

            if not numpy.array_equal(self.T.sum(1), numpy.array([1.0] * len(self.T.sum(1)))):
                raise ValueError("The sum of each row in the transition matrix should equal to 1")
        else:
    ...
    def _print_HMM(self, label, write_to_file=False):
        ...
        results = "\n" * 2 + "*" * 24 + "\n" + label + "\n" + "*" * 24 + "\n" \
                  ...
                  + "\n2) Number of observable symbols:" + str(self.V) + "\t(0: wrong, 1: correct)"\
                  + "\n3) The symbol mapping in HMM:" + str(self.symbol_map) + "\t(0: Not learned, 1: Learned)"\
    ...
    def forward(self, Obs, scaling=True, debug=False):
        ...
        Alpha[:, 0] = self.Pi     ### init with Pi (initial state prob.)
        for i in range(self.n_elements):
            Alpha[:, 0] *= self.E[i][:, int(observation[i][0])]   ### * E (emission prob.)
        ...
        # 2. Induction
        for t in range(1, T):
            Alpha[:, t] = numpy.dot(Alpha[:, t - 1], self.T)   #### alpha_{t-1} * T (transition prob.)
            for i in range(self.n_elements):
                Alpha[:, t] *= self.E[i][:, int(observation[i][t])]  #### * E (emission prob.)
    ...
    def backward(self, Obs, scaling, debug=False, **args):
        ...
        for t in reversed(range(T - 1)):
            temp = self.T.copy()          #### T: Transition prob(i,j)
            for i in range(self.n_elements):
                temp *= self.E[i][:, int(observation[i][t + 1])]  ### * E{t+1}: Emission prob{t+1}(j)

            Beta[:, t] = numpy.dot(temp, Beta[:, t + 1])  ### * beta{t+1}(j)
            if scaling:
                c[t] = 1 / Beta[:, t].sum()
                Beta[:, t] = Beta[:, t] * c[t]
    ...
    def baum_welch(self, Obs_seq, **args):
        ...
        for epoch in range(epochs):
            ...
            for Obs in Obs_seq:
                ...
                log_prob, Alpha, c = self.forward(Obs, scaling=True, debug=False)   ### Forward algorithm
                LogLikelihood += log_prob
                # print "Log Likelihood is " + str(LogLikelihood)

                Beta = self.backward(Obs, scaling=True, debug=False)    ### Backwward algorithm 
                ...
                # being in State Si at time t, given the observation sequence   --> P(q_t = s_i | O_1, ... O_T) = S_t(i)
                ...
                # and in state Sj at time t+1 ---> P(q_t=s_i, q_{t+1} = s_j | O_1, ..., O_T) = S_t(i,j)
